...
from milvus_utils import get_milvus_client, setup_milvus_collection, index_data_if_needed, search_similar_documents,search_similar_documents2
from rag_core import generate_answer,generate_answer_openai
    ...
    if collection_is_ready and models_loaded:
        ...
        if not indexing_successful and not id_to_doc_map:
            ...
            for history in st.session_state.conversation_history:
                st.info(f"**用户问题:** {history['query']}")
                st.info(f"**系统回答:** {history['answer']}")
            ...
            if st.button("获取答案", key="submit_button") and query:
                ...
                if not retrieved_ids:
                    ...
                    if not retrieved_docs:
                        ...
                        # 保存当前对话历史
                        st.session_state.conversation_history.append({"query": query, "answer": answer})
